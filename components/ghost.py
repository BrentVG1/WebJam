...
class Ghost:
    def __init__(self, x, y, ghost_type=GhostType.FOLLOWER):
        self.x = x
        self.y = y
        ...
        self.target_y = y
        self.radius = 25
        self.detection_range = 200 if ghost_type == GhostType.FOLLOWER else 150
        self.patrol_points = []
    ...
    def setup_patrol_points(self):
        if self.type == GhostType.PATROLLER:
            # Create random patrol points around the kitchen
            for _ in range(4):
    ...
    def update(self, player, footprints):
        if self.type == GhostType.FOLLOWER:
        ...
        elif self.type == GhostType.PATROLLER:
            # Move to next patrol point
            target = self.patrol_points[self.current_patrol_index]
            self.target_x, self.target_y = target
            
            # Check if reached patrol point
            if math.sqrt((self.x - target[0])**2 + (self.y - target[1])**2) < 10:
